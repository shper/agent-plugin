...
stdout = 结构化 JSON；主会话解析后由当前宿主主模型按 CONSULT-GUIDE §5 收口。
...
# ── 上下文 + 角色 prompt 模板（对齐 CONSULT-GUIDE §4）─────────────────────────
...
async def run_refine(
    ...
    *,
    ...
    """精炼形态路由（CONSULT-GUIDE §2.3）：一形态两方向，复用 reflection / review-chain 拓扑。
