...
| **非宿主底座外部批** | 主会话 | `Bash` 调 `python3 "$ROOT/ai_client/cli.py" --provider <id>` 取各外部视角（`$ROOT` 见 consult-common §3；池里每个 provider，可并发） |
...
0. **跨底座独立性检测**（C4，承 SKILL.md Step 4）：`python3 "$ROOT/ai_client/independence.py" --host <host> --pool <池>`；有重合则在综合收口里如实标注哪些外部视角同源、其卡的"独立补盲"价值打折。
...
3. **补录宿主 persona 卡留痕**（C1）：`panel.js` 是 Workflow 不能写文件，主会话收齐后把 3 张卡（JSON 或精简 markdown）经 stdin 回填 session.md：
   ```bash
   printf '%s' "<3 张宿主 persona 卡>" | python3 "$ROOT/ai_client/consult_log.py" cards --task "$TASK" --mode panel
   ```
