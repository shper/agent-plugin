...
> ```
>
> **切勿 `cd` 进 `$ROOT`**——留痕脚本据 cwd / `CLAUDE_PROJECT_DIR` 定位宿主项目，cd 会把缓存误写进插件目录（Codex 不注入项目根变量，更依赖 cwd）。配置数据区 `$CLAUDE_PLUGIN_DATA` / `$PLUGIN_DATA` 同样仅 hook 环境可靠；`ai_client/config.py` 已把数据区缺省回退到脚本同目录，故外部 key 配置不依赖该变量在 skill 环境存在。
